...
def is_mel(email):
    ...
    return email.lower() == "mel@ubermelon.com"
...
def most_and_least_common_type(treats):
    ...
    If no treats in list
        >>> treats2 = []
        >>> most_and_least_common_type(treats2)
        (None, None)

    If no least, returns both of most
        >>> treats3 = [{'type': 'dessert'}, {'type': 'dessert'}]
        >>> most_and_least_common_type(treats3)
        ('dessert', 'dessert')

    If only one item, returns as both most and least
        >>> treats4 = [{'type': 'dessert'}]
        >>> most_and_least_common_type(treats4)
        ('dessert', 'dessert')

    For tie, selects one of the tying items
    Note: adjusted to handle the ordering
        >>> treats5 = [
        ...     {'type': 'dessert'},
        ...     {'type': 'dessert'},
        ...     {'type': 'drink'},
        ...
        >>> most_and_least_common_type(treats5) in [('dessert', 'drink'), ('dessert', 'appetizer')]
        True
    ...
    for treat_type, count in sorted(types.items()):
